...
"""
Minimal set of win32 functions used by SynthEyes engine to manage toolkit UI
under Windows
"""
import ctypes
...
def safe_get_window_text(hwnd):
    ...
    try:
        ...
        result = SendMessageTimeout(hwnd, WM_GETTEXT, buffer_sz,
                                    ctypes.byref(buffer),
...
def find_windows(process_id=None, class_name=None, window_text=None,
    ...
    :param process_id: only match windows that belong to this process id
                       if specified
    :param class_name: only match windows that match this class name
                       if specified
    :param window_text: only match windows that match this window text
                        if specified
