...
class SynthEyesStartVersionControlPlugin(HookBaseClass):
    """
    Simple plugin to insert a version number into the syntheyes file path if one
    does not exist.
    """
    ...
    def accept(self, settings, item):
        ...
        if path:
            ...
            if version_number is not None:
                self.logger.info(
                    "SynthEyes '%s' plugin rejected the current session..." % (self.name,)
                )
                self.logger.info("  There is already a version number in the file...")
                self.logger.info("  SynthEyes file path: %s" % (path,))
        ...
        self.logger.info(
            "SynthEyes '%s' plugin accepted the current session." % (self.name,),
    ...
    def validate(self, settings, item):
        ...
        version_path = self.parent.util.get_version_path(path, "v001")
