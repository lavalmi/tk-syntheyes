...
class SynthEyesSessionPublishPlugin(HookBaseClass):
    """
    Plugin for publishing an open syntheyes session.
    ...
    def settings(self):
        ...
        base_settings = super(SynthEyesSessionPublishPlugin, self).settings or {}
    ...
    def validate(self, settings, item):
        ...
        # this method will handle validation specific to the syntheyes script
            ...
            self.logger.error(
                ...
                extra={
                    "action_button": {
                        ...
                        "v%s" % (version,),
                        "callback": lambda: engine.save_session_as(next_version_path),
                    }
                },
            )
            raise Exception(error_msg)
        ...
        return super(SynthEyesSessionPublishPlugin, self).validate(settings, item)
    ...
    def publish(self, settings, item):
        ...
        super(SynthEyesSessionPublishPlugin, self).publish(settings, item)
    ...
    def finalize(self, settings, item):
        ...
        super(SynthEyesSessionPublishPlugin, self).finalize(settings, item)

        # bump the session file to the next version
        self._save_to_next_version(item.properties["path"], item, self.parent.engine.save_session_as)
